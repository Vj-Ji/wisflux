...
_.map(members, (member) => fname.push(_.split(member.name, " ")[0]));
...
const newA: Array<Member> = _.filter(
  members,
...
const membersWithAge = _.filter(members, (member: Member) => member.age);
const avg =
  _.reduce(
    membersWithAge,
    (total: number, member: Member) => {
...
const elementAdded: Member[] = [{ name: "Ashu", age: 20 }, ...members];
